...
# গ্রুপ আকারে ডাটা রাখার জন্য ডিকশনারি
...
def extract_channel_name(extinf):
    """#EXTINF থেকে চ্যানেলের মূল নাম বের করার ফাংশন"""
    if ',' in extinf:
        name = extinf.split(',')[-1].strip()
        # ক্লিন করার জন্য দরকারি অংশ
        name_clean = re.sub(r'\s*\b(hd|sd|fhd|4k|720p|1080p|bdix)\b', '', name, flags=re.IGNORECASE)
    ...
    if line.startswith("#EXTINF"):
        ...
        if i < len(raw_lines):
            ...
            if stream_url not in seen_urls:
                ...
                if is_radio_ext or is_radio_kw:
                    ...
                    # চ্যানেলের নাম অনুযায়ী গ্রুপিং
                    ch_name = extract_channel_name(extinf_line)
                    channel_groups[ch_name.lower()].append((extinf_line, stream_url))
        ...
        if stream_url not in seen_urls:
            ...
            if is_radio:
                ...
                channel_groups["other"].append(("", stream_url))
    i += 1

# গ্রুপগুলোকে বর্ণানুক্রমিকভাবে (A-Z) সাজানো
